...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, train=True):
        ...
        '''
        self.keys = [x for x in (self.trainVid if train else self.testVid)] # 得到是第几个文本

        self.len = len(self.keys)

    def __getitem__(self, index):
        vid = self.keys[index] # 第几个文本
